...
- [시스템 아키텍처](#-시스템-아키텍처)
- [CI/CD & 배포](#-cicd--배포)
- [성능 최적화 (Redis 캐싱)](#-성능-최적화-redis-캐싱)
- [기술적 의사결정](#-기술적-의사결정)
- [로컬 실행](#-로컬-실행)
- [프로젝트 구조](#-프로젝트-구조)
...
자주 조회되지만 거의 변하지 않는 **문제 목록 · 문제 상세 · 태그 목록**에 Redis 캐시(Spring `@Cacheable`)를 적용했습니다. 특히 문제 목록 응답은 각 문제의 태그를 지연 로딩하며 **문제당 추가 쿼리(N+1)** 가 발생하는데(50개 목록이면 요청 1건에 최대 50여 쿼리), 캐시 히트는 이 DB 왕복을 통째로 없앱니다.

**부하 테스트(k6) 결과** — 로컬, 문제 목록(70%)/상세(30%) 혼합 시나리오

| 지표 | 캐싱 전 | 캐싱 후 | 개선 |
|---|---|---|---|
| **처리량** (50 VUs 포화) | 1,897 req/s | **18,624 req/s** | **≈ 9.8배** |
| p95 지연 (포화 시) | 43.5 ms | 4.97 ms | **−89%** |
| p95 지연 (평상 부하) | 6.36 ms | 2.43 ms | −62% |
...
> 로컬(단일 머신·소량 데이터) 기준 수치입니다. 실제 배포처럼 애플리케이션과 DB가 분리되면 쿼리마다 네트워크 왕복이 더해져 N+1 제거(=캐시) 효과는 더 커집니다.
...
- **읽기 캐시는 무효화 전략을 먼저 정하고 도입**
  "빠르게"보다 "정확하게 오래되지 않게"가 먼저라고 보고, 데이터가 바뀌는 지점(피드백→tier 재계산)을 찾아 캐시별 무효화 방식을 나눴습니다([성능 최적화](#-성능-최적화-redis-캐싱) 참고). 이미 refresh 토큰 저장소로 쓰던 Redis를 캐시 저장소로 재사용해 인프라 추가 없이 도입했습니다.
